!+ From PDAS, edited minimally by Duncan Lowther to allow for for C compatibility.
...
!  4Jan99  1.3   RLC   Made the tests for zero constant term exactly zero
  ...
  IMPLICIT NONE
  REAL(C_DOUBLE),PARAMETER,PRIVATE:: ZERO=0.0D0, FOURTH=0.25D0, HALF=0.5D0
  REAL(C_DOUBLE),PARAMETER,PRIVATE:: ONE=1.0D0, TWO=2.0D0, THREE=3.0D0, FOUR=4.0D0
  COMPLEX(C_DOUBLE_COMPLEX),PARAMETER,PRIVATE:: CZERO=(0.D0,0.D0)

  REAL(C_DOUBLE),PARAMETER,PRIVATE:: EPS=EPSILON(ONE)
...
! ---------------------------------------------------------------------------
! PURPOSE - Compute the Cube Root of a REAL(C_DOUBLE) number. If the argument is
...
!   much larger in magnitude than the other two
  ...
  REAL(C_DOUBLE),INTENT(IN):: a1,a2,a4
  REAL(C_DOUBLE),INTENT(IN):: w
  COMPLEX(C_DOUBLE_COMPLEX),INTENT(OUT),DIMENSION(:):: z


  REAL(C_DOUBLE),DIMENSION(3):: aq
!----------------------------------------------------------------------------
  aq(1)=a1
  aq(2)=a2+a1/w
  aq(3)=-a4*w
  CALL QuadraticRoots(aq, z)
  z(3)=CMPLX(w,ZERO,C_DOUBLE_COMPLEX)
  
  IF (AIMAG(z(1)) == ZERO) RETURN
  z(3)=z(2)
  z(2)=z(1)
  z(1)=CMPLX(w,ZERO,C_DOUBLE_COMPLEX)
  RETURN
END Subroutine OneLargeTwoSmall   ! -----------------------------------------

!+
SUBROUTINE QuadraticRoots(a, z) BIND(C)
! ---------------------------------------------------------------------------
! PURPOSE - COMPUTES THE ROOTS OF THE REAL POLYNOMIAL
!              A(1) + A(2)*Z + A(3)*Z**2
...
!----------------------------------------------------------------------------
  IF(a(1)==0.0) THEN     ! EPS is a global module constant (private)
    ...
    z(2) = CMPLX(-a(2)/a(3), ZERO,C_DOUBLE_COMPLEX)    ! remainder is a linear eq.
  ...
  IF (ABS(d) <= TWO*eps*a(2)*a(2)) THEN
    z(1) = CMPLX(-HALF*a(2)/a(3), ZERO, C_DOUBLE_COMPLEX) ! discriminant is tiny
    z(2) = z(1)
  ...
  IF (d < ZERO) THEN
    ...
    y = ABS(HALF*r/a(3))
    z(1) = CMPLX(x, y, C_DOUBLE_COMPLEX)
    z(2) = CMPLX(x,-y, C_DOUBLE_COMPLEX)   ! its conjugate
  ...
  IF (a(2) /= ZERO) THEN              ! see Numerical Recipes, sec. 5.5
    w = -(a(2) + SIGN(r,a(2)))
    z(1) = CMPLX(TWO*a(1)/w,  ZERO, C_DOUBLE_COMPLEX)
    z(2) = CMPLX(HALF*w/a(3), ZERO, C_DOUBLE_COMPLEX)
    outputCode=22           ! two real roots
    RETURN
  END IF

  x = ABS(HALF*r/a(3))   ! a(2)=0 if you get here
  z(1) = CMPLX( x, ZERO, C_DOUBLE_COMPLEX)
  z(2) = CMPLX(-x, ZERO, C_DOUBLE_COMPLEX)
  outputCode=22
  RETURN
END Subroutine QuadraticRoots   ! -------------------------------------------

!+
SUBROUTINE CubicRoots(a, z) BIND(C)
!----------------------------------------------------------------------------
...
!              A(1) + A(2)*Z + A(3)*Z**2 + A(4)*Z**3
  ...
  REAL(C_DOUBLE):: r, ra, rb, rq, rt
  REAL(C_DOUBLE):: r1, s, sf, sq, sum, t, tol, t1, w
  REAL(C_DOUBLE):: w1, w2, x, x1, x2, x3, y, y1, y2, y3
...
!                   ALL ROOTS ARE REAL
  ...
  z(1) = CMPLX(x1, ZERO,C_DOUBLE_COMPLEX)
  z(2) = CMPLX(x2, ZERO,C_DOUBLE_COMPLEX)
  z(3) = CMPLX(x3, ZERO,C_DOUBLE_COMPLEX)
  RETURN
...
51 IF (s < 0.1D0*ABS(w)) GO TO 70
  ...
  z(1) = CMPLX(w, ZERO,C_DOUBLE_COMPLEX)
  z(2) = CMPLX(x, y,C_DOUBLE_COMPLEX)
  z(3) = CMPLX(x,-y,C_DOUBLE_COMPLEX)
  RETURN
...
110 z(1) = CMPLX(-p, ZERO,C_DOUBLE_COMPLEX)
  w = SQRT(ABS(c))
  IF (c < ZERO) GO TO 120
  z(2) = CMPLX(-p, w,C_DOUBLE_COMPLEX)
  z(3) = CMPLX(-p,-w,C_DOUBLE_COMPLEX)
  RETURN

120 IF (p /= ZERO) GO TO 130
  z(2) = CMPLX(w, ZERO,C_DOUBLE_COMPLEX)
  z(3) = CMPLX(-w, ZERO,C_DOUBLE_COMPLEX)
  RETURN

130 x = -(p + SIGN(w,p))
  z(3) = CMPLX(x, ZERO,C_DOUBLE_COMPLEX)
  t = THREE*a(1)/(a(3)*x)
  IF (ABS(p) > ABS(t)) GO TO 131
  z(2) = CMPLX(t, ZERO,C_DOUBLE_COMPLEX)
  RETURN
131 z(2) = z(1)
  z(1) = CMPLX(t, ZERO,C_DOUBLE_COMPLEX)
  RETURN
END Subroutine CubicRoots   ! -----------------------------------------------


!+
SUBROUTINE QuarticRoots(a,z) BIND(C)
!----------------------------------------------------------------------------
! PURPOSE - Compute the roots of the real polynomial
!               A(1) + A(2)*Z + ... + A(5)*Z**4
  ...
  REAL(C_DOUBLE),DIMENSION(4):: temp
  REAL(C_DOUBLE):: u, v, v1, v2, x, x1, x2, x3, y
...
31 z(1) = CMPLX(temp(1), ZERO,C_DOUBLE_COMPLEX)
  z(2) = CMPLX(temp(2), ZERO,C_DOUBLE_COMPLEX)
  z(3) = CMPLX(temp(3), ZERO,C_DOUBLE_COMPLEX)
  z(4) = CMPLX(temp(4), ZERO,C_DOUBLE_COMPLEX)
  RETURN
...
y = v1 + v2
z(3) = CMPLX(x, y,C_DOUBLE_COMPLEX)
z(4) = CMPLX(x,-y,C_DOUBLE_COMPLEX)
RETURN
...
71 u = -x - b
  ...
  z(1) = CMPLX(x1, ZERO,C_DOUBLE_COMPLEX)
  z(2) = CMPLX(x2, ZERO,C_DOUBLE_COMPLEX)
  z(3) = CMPLX(u, v,C_DOUBLE_COMPLEX)
  z(4) = CMPLX(u,-v,C_DOUBLE_COMPLEX)
  RETURN

80 v = SQRT(ABS(t))
  z(1) = CMPLX(-b, v,C_DOUBLE_COMPLEX)
  z(2) = CMPLX(-b,-v,C_DOUBLE_COMPLEX)
  z(3) = z(1)
  z(4) = z(2)
  RETURN
...
! PURPOSE - Reorder the elements of in increasing order.
  REAL(C_DOUBLE),INTENT(IN OUT),DIMENSION(:):: a
...
SUBROUTINE SolvePolynomial(quarticCoeff, cubicCoeff, quadraticCoeff, &
  linearCoeff, constantCoeff, code, root1,root2,root3,root4) BIND(C)
! ---------------------------------------------------------------------------
  REAL(C_DOUBLE),INTENT(IN):: quarticCoeff
  REAL(C_DOUBLE),INTENT(IN):: cubicCoeff, quadraticCoeff
  REAL(C_DOUBLE),INTENT(IN):: linearCoeff, constantCoeff
  INTEGER(C_INT),INTENT(OUT):: code
  COMPLEX(C_DOUBLE_COMPLEX),INTENT(OUT):: root1,root2,root3,root4
  REAL(C_DOUBLE),DIMENSION(5):: a
  COMPLEX(C_DOUBLE_COMPLEX),DIMENSION(5):: z
!----------------------------------------------------------------------------
  a(1)=constantCoeff
  a(2)=linearCoeff
  a(3)=quadraticCoeff
...
! PURPOSE - Interchange the contents of a and b
  REAL(C_DOUBLE),INTENT(IN OUT):: a,b
  REAL(C_DOUBLE):: t
!----------------------------------------------------------------------------
  t=b
  b=a
